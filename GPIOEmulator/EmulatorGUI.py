...
class GPIO:
    ...
    def add_event_callback(channel, callback):
        channel = str(channel)
        if channel not in CALLBACKS:
            CALLBACKS[channel] = []
        CALLBACKS[channel].append(callback)


    def add_event_detect(channel, event_type, callback=None, bouncetime=0):
        channel = str(channel)
        if callback:
